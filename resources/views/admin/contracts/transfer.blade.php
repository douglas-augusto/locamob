    ...
    <div class="container">
        <div class="row">
            <div class="col s12">
                <div class="row">
                    <div class="col s8">
                        <h5 style="padding: 17px;" class="breadcrumbs-title">Repasses</h5>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col s12 m4">
                <div class="card @if($m->payed == 1) green @else white @endif  darken-1">
                    ...
                    <div class="card-action" @if($m->payed == 1) style="border-color: #fff3e0" @endif>
                        <form id="monthly_form{{$m->id}}" action="{{route('admin.contracts.transfers.update',['transfer' => $m->id,])}}" method="post">
                            @csrf
                            <div>
                                ...
                                <label for="check{{$m->id}}" style="color: @if($m->payed == 1) #fff @else #000 @endif">@if($m->payed == 1) REALIZADO @else MARCAR COMO REALIZADO @endif</label>
                            </div>
                        </form>
                    </div>
                </div>
